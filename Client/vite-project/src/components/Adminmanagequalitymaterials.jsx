// src/components/ManageQualityMaterials.js
...
import QualityMaterialForm from './Adminmanagequalitymaterialform';

const ManageQualityMaterials = () => {
      ...
      const fetchMaterials = async () => {
        try {
          const response = await axios.get(
            'http://localhost:4000/api/admin/quality-materials',
            {
              headers: {
  ...
  const handleUpdateMaterial = async (materialData) => {
    try {
      const response = await axios.put(
        `http://localhost:4000/api/admin/quality-materials/${materialData.id}`,
        materialData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="min-h-screen p-4 bg-gray-100">
      <h1 className="text-3xl font-bold mb-6">Manage Quality Materials</h1>
            ...
            <tbody>
              {materials.map((material) => (
                ...
                >
                  ...
                  <td className="py-3 px-6 text-left">{material.quality}</td>
...
export default ManageQualityMaterials;
